...
public class RedirectSystem {
   ...
   */
  private static RedirectSystem callRedirect = new RedirectSystem();
   ...
   */
  public void redirectTo(String[] input, String output) {
    /*
  ...
  public static String[] redirectionSeperator(String input) {
    ...
    if (!(Validator.validNameorPath(path, "[^A-Za-z0-9_\\/-]"))) {
      // return null whenever OUTFILE avoids correct name conventions.
      return null;
    }
    /*
     * Get OUTFILE's parent path and Validate if path leads to a Directory in the fs. If not
     * then cancel the command
     */
    String parentPath = path.substring(0, Math.max(0, path.lastIndexOf("/")));
    if (!(Validator.validatePathTo(parentPath))) {
      return null;
    }
  ...
  private void redirectToFile(String text, String path, boolean appendFlag) {
    ...
    Object parentDir = FileSystem.getInstance().pathToDirorFile(parentPath);
    /*
