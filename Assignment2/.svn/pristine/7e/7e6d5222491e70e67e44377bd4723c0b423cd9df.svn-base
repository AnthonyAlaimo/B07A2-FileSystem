...
public class Validator {
  RedirectSystem redirector = new RedirectSystem();
  ...
  public Validator(String input) {
    /*
     *  Adds the inputed command to the command history if it is not a call of
     *  the load command (since load is not supposed to be included in the
     *  command history.
     */
    if (!(input.split(" ")[0].equals("load"))) {
  ...
  /**
   * Validates if the command belongs to the list of acceptable commands that
   * were initialized at start up from JShell
   * @param inputList: Input split by every space
   * @param input: Input given by the user
   * @param redrct: The separating term given by redirection System.
   */
  private void validateCmd(String[] inputList, String input, 
      String[] redrct) {
    ...
    for (String name : Command.getCommands()) {
      if (inputList[0].toLowerCase().equals(name)) {
        ...
        redirector.redirectTo(redrct, output);
        break;
      }
    }
    /*
  ...
  public static boolean validateLeadingWhiteSpace(String input) {
    ...
    if (input.matches("^\\s+(.)*")) {
      ExceptionHandler.leadingWhiteSpacesException(input);
      return false;
    }
    return true;
  }
  ...
  /**
   * Checks if path leads somewhere in the FileSystem.getInstance()
   * 
   * @param path
   * @return
   */
  public static boolean validatePathTo(String path) {
    // Validate if parent path leads to a Directory inside the FileSystem.getInstance()
    if (FileSystem.getInstance().pathToDirorFile(path) instanceof Directory) {
      return true;
    }
    // return false if path does not lead to a Directory in the FileSystem.getInstance().
    return false;
  }
  ...
  public static boolean validateParent(Directory oldDir, Directory newDir) {
    ...
    String oldPath = FileSystem.getInstance().tracePath(oldDir, "");
    String newPath = FileSystem.getInstance().tracePath(newDir, "");
    /*
     * oldDir is a parent of newDir whenever the index of oldDir's occurrence is 0 in the path of
     * newDir.
     */
    if (newPath.indexOf(oldPath) == 0) {
      ExceptionHandler.parentPathException(
          oldPath, newPath);
      return true;
    }
    return false;
